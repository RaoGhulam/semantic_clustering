...
from sentence_dissimilarity import compute_symmetric_dissimilarity_matrix
from collections import defaultdict
from sklearn.metrics import silhouette_score


def cluster_sentences(sentences, visualize=False, max_clusters=10):
    # Step 1: Compute the symmetric dissimilarity matrix
    dissimilarity_matrix = compute_symmetric_dissimilarity_matrix(sentences)

    # Step 2: Find optimal number of clusters using silhouette score
    best_n = 2
    best_score = -1
    max_test_clusters = min(len(sentences), max_clusters)
